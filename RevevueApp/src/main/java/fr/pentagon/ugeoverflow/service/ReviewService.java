...
public class ReviewService {
    ...
    private final ReviewServiceWithFailure reviewServiceWithFailure;
    ...
    public ReviewService(QuestionRepository questionRepository,
                         ...
                         ReviewServiceWithFailure reviewServiceWithFailure) {
        ...
        this.reviewServiceWithFailure = reviewServiceWithFailure;
    }
    ...
    public ReviewUpdateDTO updateById(long reviewId, ReviewUpdateDTO reviewUpdateDTO) {
        var retry = true;
        ReviewUpdateDTO response = null;

        while (retry) {
            retry = false;

            try {
                response = reviewServiceWithFailure.updateById(reviewId, reviewUpdateDTO);
    ...
    @Transactional
    public List<UserReviewDTO> getReviewsFromUser(long userId) {
        var user = userRepository.findById(userId)
                .orElseThrow(() -> HttpException.notFound("user " + userId + " doesn't exist"));
        return reviewRepository.findByAuthorAndQuestionIsNotNullOrderByCreatedAtDesc(user)
                .stream()
                .map(reviewMapper::entityToUserReviewDTO)
                .toList();
    }
}
